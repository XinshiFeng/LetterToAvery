...
function unlockGate() {
  try { letterBody.textContent = decode(LETTER_B64); } catch {}
  gate.classList.add("gate-out");
  // Remove gate after transition; timeout is a fallback if transitionend never fires.
  const cleanup = () => { gate.style.display = "none"; };
...
// ── password gate ──────────────────────────────────────────────────────────
gateInput.addEventListener("keydown", (e) => { if (e.key === "Enter") tryPassword(); });
gateSubmit.addEventListener("click", tryPassword);
